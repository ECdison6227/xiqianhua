...
Access Secret 已通过官方 CLI 保存到本机钥匙串，并加入服务器私有环境 ZHIHU_ACCESS_SECRET。官方 auth status --verify 通过，me contents --type all --limit 1 成功读取一条记录，仅记录成功与数量，不保存用户内容。用户已完成真实授权并反馈：创作成功、关注成功、收藏夹列表返回 30001、近期收藏无记录。服务器日志确认本次 stateVerified=true。

旧版把收藏夹列表失败后的依赖查询误标为“无记录”，这一结果不能作为收藏夹内容为空的证据。已改为“未执行”，仅在收藏夹列表明确返回空数组时才显示无记录；无效响应继续报告错误，不转换为空数据。各项失败返回固定原因与安全的错误编号，不输出上游原文。运行日志仅记录状态、数量和错误类别，不记录用户内容、身份、Cookie 或令牌。官方用户数据文档把 30001 定义为频率限制；本次服务器记录已确认该编号。前端明确显示“请求过于频繁”，并暂停核验 60 秒，刷新页面保留等待时间，不自动重试。60 秒是本产品的保守等待间隔，不代表平台承诺恢复时间。此次只更新静态页面，保留现有服务器登录会话。收藏夹相关结果仍待平台恢复后复核，不能把独立 CLI 的成功当作 OAuth 接口通过。
